...
-- Migration: 20260817_007_sales_pos_orders.sql
...
create policy "sales_payments_update_member"
...
begin
  ...
  if not exists (
    ...
    execute format(
      'CREATE SEQUENCE public.%I START WITH 1 INCREMENT BY 1 NO CACHE',
      v_seq_name
    );
  end if;

  -- 4. Get next value
  execute format('SELECT nextval(public.%I)', v_seq_name)
  into v_next;

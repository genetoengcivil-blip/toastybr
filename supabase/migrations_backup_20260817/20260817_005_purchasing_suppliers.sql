...
-- Migration: 20260817_005_purchasing_suppliers.sql
...
create table if not exists public.suppliers (
  ...
  constraint suppliers_organization_check
    check (organization_id in (select id from public.organizations))
);
...
create policy "suppliers_insert_owner_admin_manager"
  ...
  with check (
    ...
    and (public.has_org_role('owner') or public.has_org_role('admin') or public.has_org_role('manager'))
  );
...
create table if not exists public.purchase_orders (
  ...
  constraint purchase_orders_organization_check
    check (organization_id in (select id from public.organizations))
);
...
create table if not exists public.purchase_order_items (
  ...
  constraint purchase_order_items_organization_check
    check (organization_id in (select id from public.organizations)),

  constraint purchase_order_items_received_lte_ordered_check
    check (quantity_received <= quantity_ordered),

  constraint purchase_order_items_ingredient_org_check
    check (ingredient_id in (select id from public.ingredients where organization_id = purchase_order_items.organization_id))
);
...
create table if not exists public.purchase_receipts (
  ...
  constraint purchase_receipts_organization_check
    check (organization_id in (select id from public.organizations))
);
...
create table if not exists public.purchase_receipt_items (
  ...
  constraint purchase_receipt_items_organization_check
    check (organization_id in (select id from public.organizations))
);
